...
| Reported failure | Automated evidence | Gate |
|---|---|---|
| A cross-monitor region drag stops at one display or maps mixed-DPI pixels to the wrong crop | The selector contract accepts one virtual-desktop rectangle across display boundaries and converts each DIP segment through its owning display scale. | `check:image-region-selector`, `check:image-desktop` |
