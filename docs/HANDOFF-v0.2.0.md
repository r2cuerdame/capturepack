...
- **NEVER touch the installed app.** `C:/Users/recue/AppData/Local/Programs/CapturePack` and `%APPDATA%/CapturePack` are live. Reading logs is fine; writing, killing, restarting, uninstalling are not. `%APPDATA%/CapturePack` holds the run marker the running app depends on. (Pre-0.3.5 profiles also hold `supervision.json`, `supervision-journal.json` and `logs/watchdog.log`; #80 orphaned them and nothing reads them. `supervision-standdown` is NOT one of those — it is the installer's live handshake with Chrome.)
...
- ~~**v0.1.6 ships a watchdog that relaunches the app** and a Start Menu fallback shortcut.~~ **Removed in 0.3.5 (#80).** Nothing relaunches CapturePack any more; the Windows login item is what starts it. Still true and still binding: anything *you* start must only ever be able to affect *your* instance, and you must tear down every process, scheduled task and shortcut you create. Verify they are gone before you finish. If you are on a machine that ever ran 0.3.4 or earlier, check for a leftover `%APPDATA%\Microsoft\Windows\Start Menu\Programs\CapturePack Capture.lnk` — Explorer holds its shortcut key until the file is deleted.
...
**Updated after #80.** Both arguments named above are gone, along with `NON_RELAUNCHABLE_ARGS` and the whole `core/src/shared/supervision.ts` that held the filter — so those two exact reproductions can no longer happen. **The underlying bug survives**: `app.relaunch()` still inherits argv, so `--capture-now`, `--simulate-no-frames`, `--show-settings` and friends still ride through a restart. Still a one-line fix, and now nothing in the tree implements it.
...
**Also open**: #78 (two identical `CapturePack.exe` in Task Manager). (#80, removing the watchdog, is DONE in 0.3.5.)
...
- **Supervision itself was rejected in 0.3.5 (#80), which settles the Scheduled Task question with it.** The earlier analysis (a task's finest repetition is one minute; it leaves Task Scheduler state an uninstall must remember; it re-implements the single-instance question; and it resurrects an app the user deliberately quit unless the quit-intent verdict is re-implemented in the startup path) is now moot in a stronger way: nothing may automatically restart CapturePack at all, because a supervisor that revives an app which died of its own bug revives it into that bug. The login item is the whole restart story.
