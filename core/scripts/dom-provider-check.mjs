// Runner for dom-provider-check.ts (#110). The check imports main-process
...
const work = mkdtempSync(path.join(tmpdir(), 'capturepack-dom-'))
try {
  ...
  execFileSync(
    ...
    [
      ...
      path.join(here, 'dom-provider-check.ts'),
      '--bundle',
      '--platform=node',
      '--format=cjs',
      `--outfile=${bundle}`,
