...
for (const path of paths) {
  ...
  if (moduleList !== undefined) {
    for (let i = 0; i < u32(moduleList); i++) {
      ...
      if (cvSize >= 24 && data.toString('ascii', cv, cv + 4) === 'RSDS') {
  ...
  if (interpretCounter) {
    if (data.readUInt16LE(system) !== 9) throw new Error('Counter interpretation requires AMD64')
    ...
    output.checkedU32Add = {
      interpretation: 'requires independently verified mov [rsi]; add [rdi+8]; checked uint32 narrowing',
