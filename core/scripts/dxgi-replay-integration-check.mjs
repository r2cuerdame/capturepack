...
check(
  'shipping recorder windows start before native READY warm-up',
...
check(
  'native bytes clear shipping cadence and own a no-op resume token',
  nativeRequest.includes('displayCadence.reset(displayId)')
    ...
    && capture.includes('if ([...nativeReplayRequests.values()].includes(displayId)) return null')
...
if (failed > 0) {
  ...
  console.log('dxgi replay integration check: 8 passed')
}
