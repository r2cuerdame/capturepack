...
export function editorUiaWindows(
  ...
  return payload.windows.map((w, index) => {
    ...
    return {
      ...
      // It is the normal route by which a reopened pack places a browser
      // document: viewport CSS pixels become snapshot pixels through
      // `client.width / viewport.width`. An affine region still also has its
      // persisted desktop-DIP crop and the page's own screen anchor; every
      // other pack without this rectangle declines, as it always did.
