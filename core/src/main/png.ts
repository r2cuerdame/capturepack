...
/**
 * The dimensions a PNG's own IHDR declares, from its first 24 bytes: signature,
 * IHDR chunk header, width and height. Null when those bytes are not a PNG.
 */
export function pngPixelSizeOf(head: Buffer): { width: number; height: number } | null {
