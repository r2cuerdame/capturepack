...
console.log('\n=== 5. cost ===')
{
  ...
  // Read from the shipping constants, never retyped here: a harness that prints
  // a budget the code no longer has is a harness that reports a passing number
  // against the wrong bar.
  console.log(
    `  frameAt()          ${frameMs.toFixed(2)} ms   ` +
      `(budget: hitTest ${M.PROVIDER_TIMEOUTS.hitTest} / frame ${M.PROVIDER_TIMEOUTS.frame} ms)`,
  )
