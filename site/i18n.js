...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
    release_note: 'Public download: 0.4.0. A pack now describes every screen, not one screen plus others, and the timeline records what moved — the cursor and the windows. Never what you typed.',
      ...
      f3_body: 'The pack records the controls that were on screen at the instant you captured — and, from every visible browser window, the page itself: every element you could see, with its role, rectangle and text. Values you typed, password boxes and anything hidden are refused on purpose, and the pack says so in its own payload.',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
    release_note: '현재 공개 다운로드는 0.4.0입니다. 이제 팩은 화면 하나에 나머지를 덧붙이는 대신 모든 화면을 기록하고, 타임라인에는 커서와 창이 어떻게 움직였는지가 남습니다. 무엇을 입력했는지는 절대 남기지 않습니다.',
      badge_foss: '무료 · 오픈소스',
      ...
      f3_body: '팩에는 캡처한 순간 화면에 있던 컨트롤이 기록되고, 보이는 모든 브라우저 창에서는 눈에 보이던 모든 요소가 역할, 사각형, 텍스트와 함께 페이지 단위로 남습니다. 입력한 값, 가려져 있던 텍스트, 화면 밖으로 스크롤된 부분은 기록을 의도적으로 거부하고, 암호 입력란은 있었다는 사실 외에 아무것도 남기지 않습니다. 무엇을 남기지 않았는지는 팩이 페이로드에 직접 적어 두므로, 비어 보이는 폼이 원래 빈 폼인지 거부된 결과인지 읽는 사람이 구분할 수 있습니다.',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
    release_note: '現在の公開ダウンロードは0.4.0です。パックは1つの画面とその他という形をやめ、すべての画面を記述するようになりました。タイムラインに残るのは動いたもの、カーソルとウィンドウです。入力した文字は決して記録しません。',
      ...
      f3_body: 'パックには取得した瞬間に画面にあったコントロールが記録され、見えているすべてのブラウザーウィンドウではページ自体も残ります。見えていたすべての要素を、ロール・矩形・テキストとともに。入力した値、パスワード欄、隠れていたものは意図的に拒否され、何を残さなかったかはパック自身のペイロードに書かれています。',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
    release_note: '当前公开下载为 0.4.0。包不再是一个屏幕加上其他，而是描述每一块屏幕；时间线记录动过的东西——光标和窗口。你敲了什么，永远不记录。',
      badge_foss: '免费开源',
      ...
      f3_body: '包会记录你捕获的那一刻屏幕上的控件；在每个可见的浏览器窗口里还会记录页面本身：你当时能看到的每个元素，连同角色、矩形和文本。你输入过的值、密码框除“它存在过”之外的一切、对你不可见的文本以及滚动到可视范围之外的内容，都被刻意拒绝记录；包也会在自身数据里写明拒绝了什么。',
    ...
    release_note: 'La descarga pública actual es 0.4.0. Un pack ya describe todas las pantallas, no una y las demás aparte, y la cronología registra lo que se movió: el cursor y las ventanas. Nunca lo que escribiste.',
      ...
      f3_body: 'El paquete registra los controles que estaban en pantalla en el instante que capturaste — y, de cada ventana de navegador visible, la página misma: cada elemento que podías ver, con su rol, rectángulo y texto. Los valores que escribiste, los campos de contraseña y todo lo oculto se rechazan a propósito, y el paquete lo dice en su propia carga.',
    ...
    release_note: 'Le téléchargement public actuel est la 0.4.0. Un pack décrit désormais tous les écrans, et non un écran plus les autres, et la chronologie retient ce qui a bougé : le curseur et les fenêtres. Jamais ce que vous avez tapé.',
      ...
      f3_body: 'Le pack enregistre les contrôles présents à l’écran à l’instant capturé — et, de chaque fenêtre de navigateur visible, la page elle-même : chaque élément visible, avec son rôle, son rectangle et son texte. Les valeurs saisies, les champs de mot de passe et tout ce qui est masqué sont refusés volontairement, et le pack le déclare dans sa propre charge utile.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.4.0. Ein Pack beschreibt jetzt jeden Bildschirm statt einen plus die anderen, und die Timeline hält fest, was sich bewegt hat — Cursor und Fenster. Nie, was Sie getippt haben.',
      ...
      f3_body: 'Das Paket speichert die Steuerelemente, die im Moment der Aufnahme auf dem Bildschirm waren — und aus jedem sichtbaren Browserfenster die Seite selbst: jedes sichtbare Element mit Rolle, Rechteck und Text. Eingegebene Werte, Passwortfelder und alles Verborgene werden absichtlich verweigert, und das Paket sagt das in seiner eigenen Nutzlast.',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
    release_note: 'O download público atual é 0.4.0. Um pack agora descreve todas as telas, não uma tela mais as outras, e a linha do tempo registra o que se moveu — o cursor e as janelas. Nunca o que você digitou.',
      ...
      f3_body: 'O pacote registra os controles que estavam na tela no instante em que você capturou — e, de cada janela de navegador visível, a própria página: cada elemento que você podia ver, com papel, retângulo e texto. Valores digitados, campos de senha e tudo o que estava oculto são recusados de propósito, e o pacote diz isso na própria carga.',
    ...
    release_note: 'Текущая публичная загрузка — 0.4.0. Пакет теперь описывает каждый экран, а не один экран и остальные отдельно, а временная шкала записывает движение курсора и окон. Что вы печатали, не записывается никогда.',
      ...
      f3_body: 'Пакет записывает элементы управления, которые были на экране в момент захвата, — а из каждого видимого окна браузера и саму страницу: каждый видимый элемент с его ролью, прямоугольником и текстом. Введённые значения, поля паролей и всё скрытое отклоняются намеренно, и пакет сообщает об этом в собственных данных.',
