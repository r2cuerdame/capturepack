...
;(function () {
  var DICT = {
    ...
    release_note: 'Public download: 0.3.3. It adds the offline pack viewer and improves replay reliability, past-object picking and multi-display context.',
      ...
      f3_body: 'The pack records the controls that were on screen at the instant you captured — and in a browser, the page itself: every element you could see, with its role, rectangle and text. Values you typed, password boxes and anything hidden are refused on purpose, and the pack says so in its own payload.',
    ...
    release_note: '현재 공개 다운로드는 0.3.3입니다. 오프라인 팩 뷰어를 추가하고 리플레이 안정성, 과거 객체 선택, 다중 디스플레이 컨텍스트를 개선했습니다.',
      ...
      f3_body: '팩에는 캡처한 순간 화면에 있던 컨트롤이 기록되고, 브라우저에서는 눈에 보이던 모든 요소가 역할, 사각형, 텍스트와 함께 페이지 단위로 남습니다. 입력한 값, 가려져 있던 텍스트, 화면 밖으로 스크롤된 부분은 기록을 의도적으로 거부하고, 암호 입력란은 있었다는 사실 외에 아무것도 남기지 않습니다. 무엇을 남기지 않았는지는 팩이 페이로드에 직접 적어 두므로, 비어 보이는 폼이 원래 빈 폼인지 거부된 결과인지 읽는 사람이 구분할 수 있습니다.',
    ...
    release_note: '現在の公開ダウンロードは0.3.3です。オフラインパックビューアーを追加し、リプレイの信頼性、過去のオブジェクト選択、マルチディスプレイのコンテキストを改善しました。',
      ...
      f3_body: 'パックには取得した瞬間に画面にあったコントロールが記録され、ブラウザーではページ自体も残ります。見えていたすべての要素を、ロール・矩形・テキストとともに。入力した値、パスワード欄、隠れていたものは意図的に拒否され、何を残さなかったかはパック自身のペイロードに書かれています。',
    ...
    release_note: '当前公开下载为 0.3.3。此版本新增离线包查看器，并改进回放可靠性、历史对象选择和多显示器上下文。',
      ...
      f3_body: '包会记录你捕获的那一刻屏幕上的控件；在浏览器里还会记录页面本身：你当时能看到的每个元素，连同角色、矩形和文本。你输入过的值、密码框除“它存在过”之外的一切、对你不可见的文本以及滚动到可视范围之外的内容，都被刻意拒绝记录；包也会在自身数据里写明拒绝了什么。',
    ...
    release_note: 'La descarga pública actual es 0.3.3. Añade el visor de paquetes sin conexión y mejora la fiabilidad de la repetición, la selección de objetos pasados y el contexto multipantalla.',
      ...
      f3_body: 'El paquete registra los controles que estaban en pantalla en el instante que capturaste — y, en un navegador, la página misma: cada elemento que podías ver, con su rol, rectángulo y texto. Los valores que escribiste, los campos de contraseña y todo lo oculto se rechazan a propósito, y el paquete lo dice en su propia carga.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.3.3. Diese Version ergänzt den Offline-Pack-Viewer und verbessert Replay-Zuverlässigkeit, frühere Objektauswahl und Multi-Display-Kontext.',
      ...
      f3_body: 'Das Paket speichert die Steuerelemente, die im Moment der Aufnahme auf dem Bildschirm waren — und im Browser die Seite selbst: jedes sichtbare Element mit Rolle, Rechteck und Text. Eingegebene Werte, Passwortfelder und alles Verborgene werden absichtlich verweigert, und das Paket sagt das in seiner eigenen Nutzlast.',
    ...
    release_note: 'O download público atual é 0.3.3. Esta versão adiciona o visualizador offline do pacote e melhora a confiabilidade do replay, a seleção de objetos anteriores e o contexto em múltiplos monitores.',
      ...
      f3_body: 'O pacote registra os controles que estavam na tela no instante em que você capturou — e, em um navegador, a própria página: cada elemento que você podia ver, com papel, retângulo e texto. Valores digitados, campos de senha e tudo o que estava oculto são recusados de propósito, e o pacote diz isso na própria carga.',
    ...
    release_note: 'Текущая публичная загрузка — 0.3.3. Версия добавляет автономный просмотр пакета и улучшает надёжность повтора, выбор объектов в прошлом и контекст нескольких дисплеев.',
      ...
      f3_body: 'Пакет записывает элементы управления, которые были на экране в момент захвата, — а в браузере и саму страницу: каждый видимый элемент с его ролью, прямоугольником и текстом. Введённые значения, поля паролей и всё скрытое отклоняются намеренно, и пакет сообщает об этом в собственных данных.',
