...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
      release_note: 'Public download: 0.3.0. The 0.3.1 source/release candidate fixes mixed-DPI Chrome DOM geometry and generated pack-document sync. The Windows build is unsigned; every release includes SHA256 checksums.',
      badge_foss: 'Free & open source',
      badge_local: 'Local-first · no cloud',
      badge_mcp: 'MCP built-in',
      demo_title: 'Rewind first, then pick.',
      demo_alt: 'Animated CapturePack editor: rewind to a past frame, select a child button, inspect its captured identity, follow its owner window, and export structured data.',
      demo_body: 'This is Object Pick in the current editor: rewind to a captured frame, hover and click the child control, inspect its captured identity and state, then scrub while its outline follows the observed owner-window movement. The same evidence is exported as structured data for AI.',
      ...
      now1: '30-second replay capture + trim', now2: 'Every display captured at once', now3: 'Object Pick with Windows UI Automation', now4: 'History, 9 languages, MCP server', now5: 'Observed owner-window tracks', now6: 'Annotated keyframes for LLMs',
      next1: 'Chrome DOM integration hardening (preview available)', next2: 'Prompt builder',
      later1: 'Sanitized sharing', later2: 'Plugin manager',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
      release_note: '현재 공개 다운로드는 0.3.0입니다. 0.3.1 소스/릴리스 후보는 혼합 DPI Chrome DOM 좌표와 생성 문서 동기화를 수정합니다. Windows 빌드는 서명되지 않았으며 모든 릴리스에 SHA256 체크섬이 포함됩니다.',
      badge_foss: '무료 · 오픈소스',
      badge_local: '로컬 우선 · 클라우드 없음',
      badge_mcp: 'MCP 내장',
      demo_title: '먼저 되감고, 그다음 선택하세요.',
      demo_alt: 'CapturePack 편집기 애니메이션: 과거 프레임으로 되감아 자식 버튼을 선택하고, 캡처된 식별 정보를 확인하고, 소유 창을 따라간 뒤 구조화 데이터를 내보냅니다.',
      demo_body: '현재 편집기의 객체 선택 기능입니다. 캡처된 프레임으로 되감아 자식 컨트롤 위에 마우스를 올리고 클릭한 뒤, 그 시점의 식별 정보와 상태를 확인하세요. 시간을 이동하면 선택선이 관측된 소유 창의 움직임을 따라가며, 같은 증거가 AI용 구조화 데이터로 저장됩니다.',
      ...
      now1: '30초 리플레이 캡처 + 트림', now2: '모든 디스플레이 동시 캡처', now3: 'Windows UI Automation 객체 선택', now4: 'History, 9개 국어, MCP 서버', now5: '관측된 소유 창 이동 트랙', now6: 'LLM용 어노테이션 키프레임',
      next1: 'Chrome DOM 통합 안정화 (프리뷰 제공)', next2: '프롬프트 빌더',
      later1: '민감정보 제거 공유', later2: '플러그인 매니저',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
      release_note: '現在の公開ダウンロードは0.3.0です。0.3.1のソース／リリース候補では混在DPIのChrome DOM座標と生成ドキュメントの同期を修正します。Windowsビルドは未署名で、各リリースにSHA256チェックサムが含まれます。',
      badge_foss: '無料 · オープンソース',
      badge_local: 'ローカルファースト · クラウドなし',
      badge_mcp: 'MCP内蔵',
      demo_title: 'まず巻き戻し、次に選択。',
      demo_alt: 'CapturePackエディターのアニメーション。過去のフレームへ巻き戻し、子ボタンを選択し、キャプチャ時の識別情報を確認し、所有ウィンドウを追跡して構造化データを書き出します。',
      demo_body: '現在のエディターで動くObject Pickです。記録済みフレームへ巻き戻し、子コントロールにカーソルを合わせてクリックし、その時点の識別情報と状態を確認します。時間を移動すると輪郭は観測済みの所有ウィンドウ移動を追い、同じ証拠がAI向け構造化データとして保存されます。',
      ...
      now1: '30秒リプレイキャプチャ + トリム', now2: '全ディスプレイ同時キャプチャ', now3: 'Windows UI AutomationのObject Pick', now4: 'History、9言語、MCPサーバー', now5: '観測済み所有ウィンドウのトラック', now6: 'LLM向け注釈キーフレーム',
      next1: 'Chrome DOM統合の安定化（プレビュー提供中）', next2: 'プロンプトビルダー',
      later1: 'サニタイズ共有', later2: 'プラグインマネージャー',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
      release_note: '当前公开下载为 0.3.0。0.3.1 源码/候选版本修复混合 DPI 下的 Chrome DOM 坐标与生成文档同步。Windows 构建未签名；每个版本都提供 SHA256 校验和。',
      badge_foss: '免费开源',
      badge_local: '本地优先 · 无云端',
      badge_mcp: '内置 MCP',
      demo_title: '先回溯，再选择。',
      demo_alt: 'CapturePack 编辑器动画：回到过去的画面，选择子按钮，查看捕获时的标识信息，跟随其所属窗口，并导出结构化数据。',
      demo_body: '这是当前编辑器中的对象选择功能：回到已捕获的画面，悬停并单击子控件，查看该时刻的标识与状态；继续移动时间时，轮廓会跟随观测到的所属窗口移动。同一份证据会保存为 AI 可读的结构化数据。',
      ...
      now1: '30 秒回放捕捉 + 裁剪', now2: '同时捕获所有显示器', now3: 'Windows UI Automation 对象选择', now4: 'History、9 种语言、MCP 服务器', now5: '观测到的所属窗口轨迹', now6: '面向 LLM 的标注关键帧',
      next1: '强化 Chrome DOM 集成（已有预览）', next2: '提示词生成器',
      later1: '脱敏分享', later2: '插件管理器',
    ...
    es: {
      ...
      release_note: 'La descarga pública actual es 0.3.0. La candidata de código/versión 0.3.1 corrige la geometría Chrome DOM con DPI mixto y la sincronización de documentos generados. La compilación de Windows no está firmada; cada versión incluye sumas SHA256.',
      badge_foss: 'Gratis y open source',
      badge_local: 'Local primero · sin nube',
      badge_mcp: 'MCP integrado',
      demo_title: 'Primero retrocede; después, selecciona.',
      demo_alt: 'Editor animado de CapturePack: retrocede a un fotograma pasado, selecciona un botón secundario, consulta su identidad capturada, sigue su ventana propietaria y exporta datos estructurados.',
      demo_body: 'Este es Object Pick en el editor actual: retrocede a un fotograma capturado, señala y selecciona el control secundario, consulta su identidad y estado en ese instante y desplázate por el tiempo mientras el contorno sigue el movimiento observado de la ventana propietaria. La misma evidencia se exporta como datos estructurados para la IA.',
      ...
      now1: 'Captura y recorte de replay de 30 s', now2: 'Captura simultánea de todas las pantallas', now3: 'Object Pick con Windows UI Automation', now4: 'History, 9 idiomas y servidor MCP', now5: 'Trayectorias observadas de la ventana propietaria', now6: 'Fotogramas clave anotados para LLM',
      next1: 'Reforzar la integración Chrome DOM (vista previa disponible)', next2: 'Generador de prompts',
      later1: 'Compartir saneado', later2: 'Gestor de plugins',
    ...
    fr: {
      ...
      release_note: 'Le téléchargement public actuel est la 0.3.0. La source/candidate 0.3.1 corrige la géométrie Chrome DOM en DPI mixtes et la synchronisation des documents générés. Le build Windows n’est pas signé ; chaque version fournit des sommes SHA256.',
      badge_foss: 'Gratuit & open source',
      badge_local: 'Local d’abord · sans cloud',
      badge_mcp: 'MCP intégré',
      demo_title: 'Remontez d’abord, sélectionnez ensuite.',
      demo_alt: 'Éditeur CapturePack animé : remontez à une image passée, sélectionnez un bouton enfant, consultez son identité capturée, suivez sa fenêtre propriétaire et exportez des données structurées.',
      demo_body: 'Voici Object Pick dans l’éditeur actuel : remontez à une image capturée, survolez puis cliquez sur le contrôle enfant, consultez son identité et son état à cet instant, puis parcourez le temps pendant que le contour suit le déplacement observé de la fenêtre propriétaire. La même preuve est exportée sous forme de données structurées pour l’IA.',
      ...
      now1: 'Capture et découpe du replay de 30 s', now2: 'Capture simultanée de tous les écrans', now3: 'Object Pick avec Windows UI Automation', now4: 'History, 9 langues et serveur MCP', now5: 'Trajectoires observées de la fenêtre propriétaire', now6: 'Images clés annotées pour les LLM',
      next1: 'Renforcer l’intégration Chrome DOM (aperçu disponible)', next2: 'Générateur de prompts',
      later1: 'Partage assaini', later2: 'Gestionnaire de plugins',
    ...
    de: {
      ...
      release_note: 'Der aktuelle öffentliche Download ist 0.3.0. Der Quell-/Release-Kandidat 0.3.1 korrigiert Chrome-DOM-Geometrie bei gemischter DPI und die Synchronisierung erzeugter Pack-Dokumente. Der Windows-Build ist nicht signiert; jede Version enthält SHA256-Prüfsummen.',
      badge_foss: 'Kostenlos & Open Source',
      badge_local: 'Local-first · keine Cloud',
      badge_mcp: 'MCP integriert',
      demo_title: 'Erst zurückspulen, dann auswählen.',
      demo_alt: 'Animierter CapturePack-Editor: zu einem früheren Bild zurückspulen, eine untergeordnete Schaltfläche wählen, ihre erfasste Identität prüfen, ihrem Besitzerfenster folgen und strukturierte Daten exportieren.',
      demo_body: 'Das ist Object Pick im aktuellen Editor: Spule zu einem erfassten Bild zurück, fahre über das untergeordnete Steuerelement und klicke es an, prüfe Identität und Zustand zu diesem Zeitpunkt und bewege dich durch die Zeit, während der Umriss der beobachteten Bewegung des Besitzerfensters folgt. Dieselben Belege werden als strukturierte KI-Daten exportiert.',
      ...
      now1: '30-Sekunden-Replay-Aufnahme + Zuschnitt', now2: 'Alle Displays gleichzeitig erfassen', now3: 'Object Pick mit Windows UI Automation', now4: 'History, 9 Sprachen und MCP-Server', now5: 'Beobachtete Spuren des Besitzerfensters', now6: 'Annotierte Schlüsselbilder für LLMs',
      next1: 'Chrome-DOM-Integration stabilisieren (Vorschau verfügbar)', next2: 'Prompt-Builder',
      later1: 'Bereinigtes Teilen', later2: 'Plugin-Manager',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
      release_note: 'O download público atual é 0.3.0. A candidata de código/versão 0.3.1 corrige a geometria Chrome DOM em DPI misto e a sincronização dos documentos gerados. O build do Windows não é assinado; cada versão inclui somas SHA256.',
      badge_foss: 'Grátis e open source',
      badge_local: 'Local-first · sem nuvem',
      badge_mcp: 'MCP embutido',
      demo_title: 'Primeiro volte; depois, selecione.',
      demo_alt: 'Editor animado do CapturePack: volte a um quadro passado, selecione um botão filho, consulte a identidade capturada, acompanhe a janela proprietária e exporte dados estruturados.',
      demo_body: 'Este é o Object Pick no editor atual: volte a um quadro capturado, passe o cursor e clique no controle filho, consulte a identidade e o estado naquele instante e percorra o tempo enquanto o contorno acompanha o movimento observado da janela proprietária. A mesma evidência é exportada como dados estruturados para a IA.',
      ...
      now1: 'Captura e corte de replay de 30 s', now2: 'Captura simultânea de todos os monitores', now3: 'Object Pick com Windows UI Automation', now4: 'History, 9 idiomas e servidor MCP', now5: 'Trilhas observadas da janela proprietária', now6: 'Quadros-chave anotados para LLMs',
      next1: 'Reforçar a integração Chrome DOM (prévia disponível)', next2: 'Gerador de prompts',
      later1: 'Compartilhamento sanitizado', later2: 'Gerenciador de plugins',
    ...
    ru: {
      ...
      release_note: 'Текущая публичная загрузка — 0.3.0. Кандидат исходного кода/выпуска 0.3.1 исправляет геометрию Chrome DOM при смешанном DPI и синхронизацию созданных документов. Сборка Windows не подписана; к каждому выпуску приложены суммы SHA256.',
      badge_foss: 'Бесплатно и open source',
      badge_local: 'Локально · без облака',
      badge_mcp: 'Встроенный MCP',
      demo_title: 'Сначала вернитесь назад, затем выберите.',
      demo_alt: 'Анимация редактора CapturePack: возврат к прошлому кадру, выбор дочерней кнопки, просмотр сохранённой идентичности, следование за окном-владельцем и экспорт структурированных данных.',
      demo_body: 'Это Object Pick в текущем редакторе: вернитесь к сохранённому кадру, наведите указатель и выберите дочерний элемент, проверьте его идентичность и состояние в тот момент, затем перемещайтесь по времени — контур последует за наблюдавшимся движением окна-владельца. Те же доказательства экспортируются как структурированные данные для ИИ.',
      ...
      now1: 'Захват и обрезка 30-секундного повтора', now2: 'Одновременный захват всех дисплеев', now3: 'Object Pick через Windows UI Automation', now4: 'History, 9 языков и MCP-сервер', now5: 'Наблюдавшиеся траектории окна-владельца', now6: 'Аннотированные ключевые кадры для LLM',
      next1: 'Стабилизация интеграции Chrome DOM (доступно превью)', next2: 'Генератор промптов',
      later1: 'Очищенный шеринг', later2: 'Менеджер плагинов',
