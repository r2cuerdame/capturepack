...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
      release_note: 'Public download: 0.3.1. This release fixes mixed-DPI Chrome DOM geometry and generated pack-document sync. The Windows build is unsigned; every release includes SHA256 checksums.',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
      release_note: '현재 공개 다운로드는 0.3.1입니다. 이 릴리스는 혼합 DPI Chrome DOM 좌표와 생성 문서 동기화를 수정합니다. Windows 빌드는 서명되지 않았으며 모든 릴리스에 SHA256 체크섬이 포함됩니다.',
      badge_foss: '무료 · 오픈소스',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
      release_note: '現在の公開ダウンロードは0.3.1です。このリリースでは混在DPIのChrome DOM座標と生成ドキュメントの同期を修正します。Windowsビルドは未署名で、各リリースにSHA256チェックサムが含まれます。',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
      release_note: '当前公开下载为 0.3.1。此版本修复混合 DPI 下的 Chrome DOM 坐标与生成文档同步。Windows 构建未签名；每个版本都提供 SHA256 校验和。',
      badge_foss: '免费开源',
    ...
    es: {
      ...
      release_note: 'La descarga pública actual es 0.3.1. Esta versión corrige la geometría Chrome DOM con DPI mixto y la sincronización de documentos generados. La compilación de Windows no está firmada; cada versión incluye sumas SHA256.',
    ...
    fr: {
      ...
      release_note: 'Le téléchargement public actuel est la 0.3.1. Cette version corrige la géométrie Chrome DOM en DPI mixtes et la synchronisation des documents générés. Le build Windows n’est pas signé ; chaque version fournit des sommes SHA256.',
    ...
    de: {
      ...
      release_note: 'Der aktuelle öffentliche Download ist 0.3.1. Diese Version korrigiert Chrome-DOM-Geometrie bei gemischter DPI und die Synchronisierung erzeugter Pack-Dokumente. Der Windows-Build ist nicht signiert; jede Version enthält SHA256-Prüfsummen.',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
      release_note: 'O download público atual é 0.3.1. Esta versão corrige a geometria Chrome DOM em DPI misto e a sincronização dos documentos gerados. O build do Windows não é assinado; cada versão inclui somas SHA256.',
    ...
    ru: {
      ...
      release_note: 'Текущая публичная загрузка — 0.3.1. Эта версия исправляет геометрию Chrome DOM при смешанном DPI и синхронизацию созданных документов. Сборка Windows не подписана; к каждому выпуску приложены суммы SHA256.',
