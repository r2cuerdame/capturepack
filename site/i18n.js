...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
    release_note: 'Public download: 0.4.3. A saved pack’s browser page can now be read back — reopening a capture recovered none of it before — and what object picking offers is measured, not assumed.',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
    release_note: '현재 공개 다운로드는 0.4.3입니다. 저장된 팩의 브라우저 페이지를 이제 다시 읽어올 수 있습니다. 지금까지는 캡처를 다시 열어도 아무것도 되살아나지 않았습니다. 객체 선택이 무엇을 내주는지도 이제 짐작이 아니라 측정으로 확인합니다.',
      badge_foss: '무료 · 오픈소스',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
    release_note: '現在の公開ダウンロードは0.4.3です。保存したパックのブラウザーページを、ようやく読み戻せるようになりました。これまではキャプチャを開き直しても何も戻ってきませんでした。オブジェクト選択が何を差し出すかも、推測ではなく計測で確かめます。',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
    release_note: '当前公开下载为 0.4.3。保存下来的包，里面的浏览器页面终于能读回来了——此前重新打开一次捕获，页面什么也剩不下；对象选择给出的范围也不再靠猜，而是有实测。',
      badge_foss: '免费开源',
    ...
    release_note: 'La descarga pública actual es 0.4.3. La página de navegador guardada en un pack por fin se puede volver a leer — al reabrir una captura no se recuperaba nada — y lo que ofrece la selección de objetos ahora se mide, no se supone.',
    ...
    release_note: 'Le téléchargement public actuel est la 0.4.3. La page web enregistrée dans un pack se relit enfin — rouvrir une capture n’en restituait rien — et ce que propose la sélection d’objet est désormais mesuré, non supposé.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.4.3. Die im Pack gespeicherte Browserseite lässt sich endlich wieder lesen — beim erneuten Öffnen einer Aufnahme kam davon nichts zurück — und was die Objektauswahl anbietet, wird gemessen statt vermutet.',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
    release_note: 'O download público atual é 0.4.3. A página de navegador guardada no pack enfim pode ser lida de volta — reabrir uma captura não recuperava nada dela — e o que a seleção de objeto oferece agora é medido, não presumido.',
    ...
    release_note: 'Текущая публичная загрузка — 0.4.3. Сохранённую в пакете веб-страницу наконец можно прочитать: раньше при повторном открытии захвата от неё не оставалось ничего. А то, что предлагает выбор объекта, теперь измеряется, а не угадывается.',
