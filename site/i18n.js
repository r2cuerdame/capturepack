...
;(function () {
  var DICT = {
    en: {
      ...
      now1: '30-second replay capture + trim', now2: 'Toolless annotation, scrub through time', now3: 'Folder packs + annotated replay', now4: 'History, 9 languages, MCP server',
      next1: 'Capture every display at once', next2: 'Semantic object picking (UIA)', next3: 'Chrome DOM capture extension', next4: 'Annotated keyframes for LLMs',
      later1: 'Unity · Unreal · VS Code · Git plugins', later2: 'Frame-accurate element tracking', later3: 'Prompt builder', later4: 'Sanitized sharing', later5: 'Plugin manager',
    ...
    ko: {
      ...
      now1: '30초 리플레이 캡처 + 트림', now2: '툴 없는 어노테이션, 시간 스크럽', now3: '폴더 팩 + 어노테이션 영상', now4: 'History, 9개 국어, MCP 서버',
      next1: '모든 디스플레이 동시 캡처', next2: '의미 객체 선택 (UIA)', next3: 'Chrome DOM 캡처 확장', next4: 'LLM용 어노테이션 키프레임',
      later1: 'Unity · Unreal · VS Code · Git 플러그인', later2: '프레임 단위 요소 추적', later3: '프롬프트 빌더', later4: '민감정보 제거 공유', later5: '플러그인 매니저',
    ...
    ja: {
      ...
      now1: '30秒リプレイキャプチャ + トリム', now2: 'ツールレス注釈、時間スクラブ', now3: 'フォルダパック + 注釈付き動画', now4: 'History、9言語、MCPサーバー',
      next1: '全ディスプレイ同時キャプチャ', next2: 'セマンティックオブジェクト選択 (UIA)', next3: 'Chrome DOMキャプチャ拡張', next4: 'LLM向け注釈キーフレーム',
      later1: 'Unity · Unreal · VS Code · Gitプラグイン', later2: 'フレーム精度の要素トラッキング', later3: 'プロンプトビルダー', later4: 'サニタイズ共有', later5: 'プラグインマネージャー',
