...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
    release_note: 'Public download: 0.3.3. It adds the offline pack viewer and improves replay reliability, past-object picking and multi-display context.',
      ...
      f3_title: '3 · Keep what the frame contained',
      f3_body: 'The pack records the controls that were on screen at the instant you picked — and in a browser, the page itself: every element you could see, with its role, rectangle and text. Values you typed, password boxes and anything hidden are refused on purpose, and the pack says so in its own payload.',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
    release_note: '현재 공개 다운로드는 0.3.3입니다. 오프라인 팩 뷰어를 추가하고 리플레이 안정성, 과거 객체 선택, 다중 디스플레이 컨텍스트를 개선했습니다.',
      badge_foss: '무료 · 오픈소스',
      ...
      f3_title: '3 · 그 프레임에 있던 것을 그대로 보관',
      f3_body: '팩에는 선택한 순간 화면에 있던 컨트롤이 기록되고, 브라우저에서는 눈에 보이던 모든 요소가 역할, 사각형, 텍스트와 함께 페이지 단위로 남습니다. 입력한 값, 가려져 있던 텍스트, 화면 밖으로 스크롤된 부분은 기록을 의도적으로 거부하고, 암호 입력란은 있었다는 사실 외에 아무것도 남기지 않습니다. 무엇을 남기지 않았는지는 팩이 페이로드에 직접 적어 두므로, 비어 보이는 폼이 원래 빈 폼인지 거부된 결과인지 읽는 사람이 구분할 수 있습니다.',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
    release_note: '現在の公開ダウンロードは0.3.3です。オフラインパックビューアーを追加し、リプレイの信頼性、過去のオブジェクト選択、マルチディスプレイのコンテキストを改善しました。',
      ...
      f3_title: '3 · そのフレームにあったものを残す',
      f3_body: 'パックは、選んだ瞬間に画面にあったコントロールを記録します。ブラウザーではページ自体も記録し、目に見えていた要素すべてを役割、矩形、テキストとともに残します。入力した値、パスワード欄（欄があったことだけを残します）、見えていなかったテキスト、表示領域の外へスクロールしていたものは意図的に拒否します。何を除いたかはパック自身のペイロードに記載されます。',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
    release_note: '当前公开下载为 0.3.3。此版本新增离线包查看器，并改进回放可靠性、历史对象选择和多显示器上下文。',
      badge_foss: '免费开源',
      ...
      f3_title: '3 · 保留那一帧里的内容',
      f3_body: '包会记录你选择的那一刻屏幕上的控件；在浏览器里还会记录页面本身：你当时能看到的每个元素，连同角色、矩形和文本。你输入过的值、密码框除“它存在过”之外的一切、对你不可见的文本以及滚动到可视范围之外的内容，都被刻意拒绝记录；包也会在自身数据里写明拒绝了什么。',
    ...
    release_note: 'La descarga pública actual es 0.3.3. Añade el visor de paquetes sin conexión y mejora la fiabilidad de la repetición, la selección de objetos pasados y el contexto multipantalla.',
      ...
      f3_title: '3 · Conserva lo que contenía el fotograma',
      f3_body: 'El pack registra los controles que estaban en pantalla en el instante elegido y, en un navegador, la página tal como se veía: cada elemento visible, con su rol, su rectángulo y su texto. Los valores que escribiste, los campos de contraseña y lo que estaba oculto o fuera de la vista se rechazan a propósito; de una contraseña solo queda constancia de que el campo estaba ahí. El pack enumera en sus propios datos lo que dejó fuera, así que un formulario vacío se distingue de uno censurado.',
    ...
    release_note: 'Le téléchargement public actuel est la 0.3.3. Cette version ajoute la visionneuse de pack hors ligne et améliore la fiabilité du replay, la sélection d’objets passés et le contexte multi-écrans.',
      ...
      f3_title: '3 · Conservez ce que contenait l’image',
      f3_body: 'Le pack enregistre les contrôles présents à l’écran à l’instant choisi et, dans un navigateur, la page elle-même : chaque élément que vous pouviez voir, avec son rôle, son rectangle et son texte. Les valeurs que vous avez saisies, les champs de mot de passe et tout ce qui vous était caché ou hors de vue sont refusés délibérément ; le pack le déclare dans ses propres données.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.3.3. Diese Version ergänzt den Offline-Pack-Viewer und verbessert Replay-Zuverlässigkeit, frühere Objektauswahl und Multi-Display-Kontext.',
      ...
      f3_title: '3 · Behalten, was im Bild war',
      f3_body: 'Das Pack speichert die Steuerelemente, die im Moment der Auswahl auf dem Bildschirm waren – im Browser zusätzlich die Seite selbst: jedes sichtbare Element mit Rolle, Rechteck und Text. Eingegebene Werte, Passwortfelder und alles, was verborgen oder aus dem sichtbaren Bereich gescrollt ist, verweigert CapturePack bewusst; das Pack vermerkt in seinen eigenen Daten, was es weggelassen hat.',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
    release_note: 'O download público atual é 0.3.3. Esta versão adiciona o visualizador offline do pacote e melhora a confiabilidade do replay, a seleção de objetos anteriores e o contexto em múltiplos monitores.',
      ...
      f3_title: '3 · Guarde o que o quadro continha',
      f3_body: 'O pack registra os controles que estavam na tela no instante escolhido e, num navegador, a própria página: cada elemento que você podia ver, com papel, retângulo e texto. Os valores que você digitou, o texto oculto e o que ficou fora da área visível são recusados de propósito; de um campo de senha fica só o registro de que ele existia. O pack declara no próprio payload o que deixou de fora.',
    ...
    release_note: 'Текущая публичная загрузка — 0.3.3. Версия добавляет автономный просмотр пакета и улучшает надёжность повтора, выбор объектов в прошлом и контекст нескольких дисплеев.',
      ...
      f3_title: '3 · Сохраните то, что было в кадре',
      f3_body: 'Пакет записывает элементы, которые были на экране в момент выбора, а в браузере — саму страницу: каждый видимый элемент с его ролью, прямоугольником и текстом. Введённые вами значения, поля пароля и всё, что скрыто или ушло за пределы видимой области, пакет намеренно отказывается записывать и сам перечисляет пропущенное в своих данных.',
