...
;(function () {
  var DICT = {
    en: {
      ...
      btn_sponsor: '♥ Sponsor',
    release_note: 'Public download: 0.5.1. From History, create a reviewed, still-only Share Copy without originals, videos, or structured capture context.',
    ...
    ko: {
      ...
      btn_download: '다운로드',
      btn_sponsor: '♥ 후원',
    release_note: '현재 공개 다운로드는 0.5.1입니다. 기록에서 검토를 거친 정지 화면 전용 공유 복사본을 만들 수 있으며, 원본·영상·구조화된 캡처 맥락은 포함되지 않습니다.',
      badge_foss: '무료 · 오픈소스',
      ...
      privacy_body: 'CapturePack은 캡처나 충돌 보고서를 업로드하지 않습니다. 앱과 이 사이트는 UTC 날짜 기준 하루 한 번 PurplePulse에 임의 설치 ID, 버전, OS, 플랫폼만 보내며 사용자 이름, 기기 이름, 캡처 데이터는 보내지 않습니다. 블러는 비파괴 방식이므로 주석 결과물은 보호되지만 전체 팩의 snapshot.png와 원본 리플레이는 가려지지 않은 상태로 남습니다.',
    ...
    ja: {
      ...
      btn_download: 'ダウンロード',
      btn_sponsor: '♥ スポンサー',
    release_note: '現在の公開ダウンロードは0.5.1です。履歴から、確認済みの静止画だけの共有コピーを作成でき、原本、動画、構造化されたキャプチャコンテキストは含まれません。',
      ...
      privacy_body: 'CapturePackはキャプチャやクラッシュレポートをアップロードしません。アプリとこのサイトはUTC日付で1日1回、ランダムなインストールID、バージョン、OS、プラットフォームだけをPurplePulseへ送信し、ユーザー名、端末名、キャプチャデータは送りません。ぼかしは非破壊で、注釈済みビューは保護されますが、フルパック内のsnapshot.pngと元のリプレイは未編集のまま残ります。',
    ...
    zh: {
      ...
      btn_download: '下载',
      btn_sponsor: '♥ 赞助',
    release_note: '当前公开下载为 0.5.1。现在可从历史记录创建经过检查、仅含静态图像的共享副本，其中不包含原始文件、视频或结构化捕获上下文。',
      badge_foss: '免费开源',
      ...
      privacy_body: 'CapturePack 不会上传捕获内容或崩溃报告。应用和本网站每天按 UTC 日期仅向 PurplePulse 发送一次随机安装 ID、版本、操作系统和平台，绝不发送用户名、设备名或捕获数据。模糊是非破坏性的：标注视图会被保护，但完整包中的 snapshot.png 和原始回放仍保留未遮挡内容。',
    ...
    release_note: 'La descarga pública actual es 0.5.1. Desde el Historial se puede crear una copia para compartir, revisada y solo con imágenes estáticas, sin originales, vídeos ni contexto estructurado de la captura.',
    ...
    release_note: 'Le téléchargement public actuel est la version 0.5.1. Depuis l’historique, créez une copie de partage vérifiée contenant uniquement des images fixes, sans originaux, vidéos ni contexte structuré de capture.',
      ...
      privacy_body: 'CapturePack n’envoie ni captures ni rapports de plantage. Une fois par jour UTC, l’application et ce site envoient à PurplePulse uniquement un identifiant d’installation aléatoire, la version, le système d’exploitation et la plateforme — jamais de nom d’utilisateur, de nom d’appareil ni de données de capture. Le flou est non destructif : il protège les vues annotées, mais snapshot.png et le replay original du pack complet restent non caviardés.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.5.1. Im Verlauf lässt sich eine geprüfte Share Copy nur mit Standbildern erstellen — ohne Originale, Videos oder strukturierten Aufnahmekontext.',
      ...
      privacy_body: 'CapturePack lädt keine Aufnahmen oder Absturzberichte hoch. Einmal pro UTC-Tag senden die App und diese Website nur eine zufällige Installations-ID, Version, Betriebssystem und Plattform an PurplePulse — niemals Benutzername, Gerätename oder Aufnahmedaten. Unschärfe ist nicht destruktiv: annotierte Ansichten sind geschützt, doch snapshot.png und das Original-Replay im vollständigen Pack bleiben ungeschwärzt.',
    ...
    pt: {
      ...
      btn_download: 'Baixar',
      btn_sponsor: '♥ Apoiar',
    release_note: 'O download público atual é 0.5.1. No Histórico, crie uma Cópia de Compartilhamento revisada, apenas com imagens estáticas, sem originais, vídeos nem contexto estruturado da captura.',
    ...
    release_note: 'Текущая публичная версия — 0.5.1. Из истории можно создать проверенную копию для отправки только со снимками — без оригиналов, видео и структурированного контекста захвата.',
