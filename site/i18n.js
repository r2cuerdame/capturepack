...
;(function () {
  var DICT = {
    ...
    release_note: 'Public download: 0.3.3. It adds the offline pack viewer and improves replay reliability, past-object picking and multi-display context.',
      ...
      f3_body: 'The pack records the controls that were on screen at the instant you picked — including inside applications built on Chromium, which a frame-rate budget used to skip. Every rectangle names the display it belongs to, so a multi-monitor pack keeps its coordinates when reopened.',
    ...
    release_note: '현재 공개 다운로드는 0.3.3입니다. 오프라인 팩 뷰어를 추가하고 리플레이 안정성, 과거 객체 선택, 다중 디스플레이 컨텍스트를 개선했습니다.',
      ...
      f3_body: '팩에는 선택한 순간 화면에 있던 컨트롤이 기록됩니다. 예전에는 프레임 레이트를 맞추느라 건너뛰던 Chromium 기반 앱 내부의 컨트롤도 포함됩니다. 모든 사각형은 자신이 속한 디스플레이를 명시하므로 여러 모니터에서 만든 팩도 다시 열었을 때 좌표가 유지됩니다.',
    ...
    release_note: '現在の公開ダウンロードは0.3.3です。オフラインパックビューアーを追加し、リプレイの信頼性、過去のオブジェクト選択、マルチディスプレイのコンテキストを改善しました。',
      ...
      f3_body: 'パックは、選んだ瞬間に画面にあったコントロールを記録します。フレームレートを保つために従来は省いていた、Chromiumベースのアプリの内部も含みます。すべての矩形が属するディスプレイを示すため、開き直したマルチモニターパックでも座標が保たれます。',
    ...
    release_note: '当前公开下载为 0.3.3。此版本新增离线包查看器，并改进回放可靠性、历史对象选择和多显示器上下文。',
      ...
      f3_body: '包会记录你选择的那一刻屏幕上的控件，基于 Chromium 构建的应用内部也包括在内；这些控件过去为控制帧率开销而被跳过。每个矩形都标明所属显示器，因此多显示器包重新打开后仍保持坐标。',
    ...
    release_note: 'La descarga pública actual es 0.3.3. Añade el visor de paquetes sin conexión y mejora la fiabilidad de la repetición, la selección de objetos pasados y el contexto multipantalla.',
      ...
      f3_body: 'El pack registra los controles que estaban en pantalla en el instante elegido, incluidos los de aplicaciones basadas en Chromium, que antes se omitían para mantener la tasa de fotogramas. Cada rectángulo identifica la pantalla a la que pertenece, así que un pack multimonitor conserva sus coordenadas al reabrirse.',
    ...
    release_note: 'Le téléchargement public actuel est la 0.3.3. Cette version ajoute la visionneuse de pack hors ligne et améliore la fiabilité du replay, la sélection d’objets passés et le contexte multi-écrans.',
      ...
      f3_body: 'Le pack enregistre les contrôles présents à l’écran à l’instant choisi, y compris dans les applications basées sur Chromium, que la limite de temps par image laissait auparavant de côté. Chaque rectangle nomme l’écran auquel il appartient : un pack multiécran garde donc ses coordonnées à la réouverture.',
    ...
    release_note: 'Der aktuelle öffentliche Download ist 0.3.3. Diese Version ergänzt den Offline-Pack-Viewer und verbessert Replay-Zuverlässigkeit, frühere Objektauswahl und Multi-Display-Kontext.',
      ...
      f3_body: 'Das Pack speichert die Steuerelemente, die im Moment der Auswahl auf dem Bildschirm waren – auch in Anwendungen auf Chromium-Basis, die CapturePack früher übersprang, um die Bildrate zu halten. Jedes Rechteck nennt das Display, zu dem es gehört, sodass ein Multi-Monitor-Pack beim erneuten Öffnen seine Koordinaten behält.',
    ...
    release_note: 'O download público atual é 0.3.3. Esta versão adiciona o visualizador offline do pacote e melhora a confiabilidade do replay, a seleção de objetos anteriores e o contexto em múltiplos monitores.',
      ...
      f3_body: 'O pack registra os controles que estavam na tela no instante escolhido, inclusive dentro de aplicativos baseados em Chromium; antes, um limite de taxa de quadros os deixava de fora. Cada retângulo identifica sua tela, preservando as coordenadas quando um pack multimonitor é reaberto.',
    ...
    release_note: 'Текущая публичная загрузка — 0.3.3. Версия добавляет автономный просмотр пакета и улучшает надёжность повтора, выбор объектов в прошлом и контекст нескольких дисплеев.',
      ...
      f3_body: 'Пакет записывает элементы, которые были на экране в момент выбора, включая элементы внутри приложений на Chromium — раньше их пропускали ради частоты кадров. Каждый прямоугольник называет свой дисплей, поэтому многомониторный пакет сохраняет координаты при повторном открытии.',
