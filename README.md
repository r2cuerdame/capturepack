...
Current public Windows release: **CapturePack 0.4.0**. A pack now describes every
screen rather than one screen plus others, and the timeline records what moved —
the cursor and the windows. Never what you typed.
...
**0.4.0 is the current public Windows download.** CapturePack remains an
...
CapturePack `0.4.0` is the application version. Pack `format_version` evolves
