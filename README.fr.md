...
Version Windows publique actuelle : **CapturePack 0.3.5**. L’application ne tourne
plus que dans un seul processus — celui qui la surveillait a disparu. Les captures
font le ménage toutes seules, selon la règle de conservation que vous fixez, et c’est
vous qui tapez le numéro d’un cadre, non plus l’application qui le décide.
...
**0.3.5 est la version Windows publique actuellement téléchargeable.** CapturePack
...
CapturePack `0.3.5` est la version de l’application. Le `format_version` des packs
